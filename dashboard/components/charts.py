...
def churn_distribution_pie(df):
    ...
    fig = px.pie(
        ...
        color="churn", color_discrete_map={"Yes": "#d62728", "No": "#1f77b4"},
...
def risk_segment_bar(df):
    ...
    counts = df["risk_segment"].value_counts().reindex(["High", "Medium", "Low"]).dropna().reset_index()
    counts.columns = ["risk_segment", "count"]

    fig = px.bar(
        counts, x="risk_segment", y="count",
        color="risk_segment", color_discrete_map=RISK_COLOR_MAP,
