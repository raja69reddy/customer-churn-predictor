...
from dashboard.components.metrics import display_kpi_row, format_number, format_percentage
...
def load_filtered_customers(filters: dict) -> pd.DataFrame:
    ...
    """
    params = {"tenure_min": filters["tenure_range"][0], "tenure_max": filters["tenure_range"][1]}
...
def main() -> None:
    ...
    display_kpi_row([
        {"title": "Overall Churn Rate", "value": format_percentage(churn_rate), "color": "#d62728"},
        {"title": "Total High Risk Customers", "value": format_number(high_risk_count), "color": "#ff7f0e"},
        {"title": "Revenue at Risk", "value": f"${revenue_at_risk:,.2f}", "color": "#9467bd"},
        {
            "title": "Model Accuracy",
            "value": format_percentage(model_accuracy) if model_accuracy is not None else "N/A",
            "color": "#2ca02c",
        },
    ])
    ...
    with col2:
        ...
        fig = px.bar(
            contract_data, x="segment", y="churn_rate_pct",
            title="Churn Rate by Contract Type", color_discrete_sequence=["#1f77b4"],
            text="churn_rate_pct",
        )
    ...
    if not tenure_data.empty:
        import plotly.express as px
        fig2 = px.bar(
            tenure_data, x="segment", y="churn_rate_pct",
            title="Churn Rate by Tenure Group", color_discrete_sequence=["#1f77b4"],
            text="churn_rate_pct",
        )
