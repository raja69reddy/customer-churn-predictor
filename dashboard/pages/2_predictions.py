...
def main() -> None:
    ...
    display_kpi_row([
        {"title": "High Risk Count", "value": format_number(int(counts.get("High", 0))), "color": "#d62728"},
        {"title": "Medium Risk Count", "value": format_number(int(counts.get("Medium", 0))), "color": "#ff7f0e"},
        {"title": "Low Risk Count", "value": format_number(int(counts.get("Low", 0))), "color": "#2ca02c"},
        {"title": "Avg Churn Probability", "value": format_percentage(avg_probability), "color": "#1f77b4"},
    ])
    ...
    high_risk = predictions[predictions["risk_segment"] == "High"][
        ["customer_id", "churn_probability", "risk_segment", "contract", "tenure", "monthly_charges", "predicted_at"]
    ...
    if search_id:
        high_risk = high_risk[high_risk["customer_id"].str.contains(search_id, case=False, na=False)]
