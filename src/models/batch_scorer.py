...
def load_production_predictor() -> tuple[ChurnPredictor, str]:
    ...
    if not production:
        raise RuntimeError(
            f"No Production version found for '{model_name}' in MLflow registry."
        )
...
def classify_risk(
    probability: float, high_threshold: float, medium_threshold: float
) -> str:
...
def score_in_chunks(
    ...
    chunks = [
        customers.iloc[i : i + CHUNK_SIZE] for i in range(0, len(customers), CHUNK_SIZE)
    ]
    scored_chunks = []
...
def run(mode: str = "full", threshold: float = None, output: str = "both") -> None:
    start = time.time()
    log.info(
        "Starting batch scoring — mode=%s, threshold=%s, output=%s",
        mode,
        threshold,
        output,
    )
    ...
    medium_threshold = (
        min(DEFAULT_MEDIUM_THRESHOLD, high_threshold / 2)
        if threshold is not None
        else DEFAULT_MEDIUM_THRESHOLD
    )
    ...
    summary = pd.DataFrame(
        [
            ...
            {
                "metric": "db_rows_saved",
                "value": db_rows if db_rows is not None else "-",
            },
    ...
    print(
        top10[["customer_id", "churn_probability", "risk_segment"]].to_string(
            index=False
        )
    )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Batch-score customers for churn risk."
    )
    parser.add_argument(
        "--mode",
        choices=["full", "new"],
        default="full",
        help="Score all customers or only unscored ones.",
    )
    parser.add_argument(
        "--threshold",
        type=float,
        default=None,
        help="Custom High-risk probability threshold (default 0.7).",
    )
    parser.add_argument(
        "--output",
        choices=["csv", "db", "both"],
