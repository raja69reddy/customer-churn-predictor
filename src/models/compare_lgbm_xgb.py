...
def run() -> None:
    ...
    xgb_metrics = evaluator.evaluate_model(
        xgboost_model, trainer.X_test, trainer.y_test
    )
    lgbm_metrics = evaluator.evaluate_model(
        lightgbm_model, trainer.X_test, trainer.y_test
    )
    ...
    comparison = pd.DataFrame(
        {
            ...
            "XGBoost": [
                xgb_metrics[m] for m in ["accuracy", "auc", "f1", "precision", "recall"]
            ],
            "LightGBM": [
                lgbm_metrics[m]
                for m in ["accuracy", "auc", "f1", "precision", "recall"]
            ],
        }
    )
    ...
    evaluator.plot_roc_curve(
        xgboost_model, trainer.X_test, trainer.y_test, "XGBoost (tuned)", ax=ax
    )
    evaluator.plot_roc_curve(
        lightgbm_model, trainer.X_test, trainer.y_test, "LightGBM", ax=ax
    )
